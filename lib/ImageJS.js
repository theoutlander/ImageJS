...
ImageJS.prototype = {
    ...
    computeGaussian: function(overlap, density, filter) {
        this.computeEightNeighborhoodDensity(overlap, density, filter);
    },

    computeEightNeighborhoodDensity: function(overlap, density, filter) {
        var data = overlap ? this.imageData.data : new Uint8ClampedArray(this.imageData.data);
        ...
        for(var c=0; c<4; c++) {
            for (var row = 0; row < this.imageData.height; row++) {
                var rowOffset = row * this.imageData.width;
                for (var col = 0; col < this.imageData.width; col++) {
                    var result = this.eightNeighborhoodDensity(rowOffset + col, c, density);

                    if(filter) {
                        var arrFilter = result.arr.map(function (val, index) {
                            return filter[index] * val;
                        });
                        data[(rowOffset*4)+(col*4)+c] = arrFilter.reduce(function(a, b) { return a+b;}) / result.arr.length;
                    }
                    else
                    {
                        data[(rowOffset*4)+(col*4)+c] = result.arr.reduce(function(a, b) { return a+b;}) / result.arr.length;
        ...
        console.log("Done with Eight Neighborhood Density " + density + " Computation!");
    },
    ...
    eightNeighborhoodDensity: function(i, channel, density) {
        var result = {
            arr: []
        }
        ...
        var length = (density - 1) / 2;

        for(i=1; i<=length; i++) {
            // North West
            if ((indexRow - i) >= 0 && (indexCol - i) >= 0) {
                result.northwest = this.imageData.data[((indexRow - i) * rowLength) + ((indexCol - i) * 4) + channel];
                result.arr.push(result.northwest);
            }

            // North
            if ((indexRow - i) >= 0) {
                result.north = this.imageData.data[((indexRow - i) * rowLength) + (indexCol * 4) + channel];
                result.arr.push(result.north);
            }

            // North East
            if ((indexRow - i) >= 0 && (indexCol + i) < cols) {
                result.northeast = this.imageData.data[((indexRow - i) * rowLength) + ((indexCol + i) * 4) + channel];
                result.arr.push(result.northeast);
            }

            // West
            if ((indexCol - i) >= 0) {
                result.west = this.imageData.data[(indexRow * rowLength) + ((indexCol - i) * 4) + channel];
                result.arr.push(result.west);
            }
            ...
            // East
            if ((indexCol + i) < cols) {
                result.east = this.imageData.data[(indexRow * rowLength) + ((indexCol + i) * 4) + channel];
                result.arr.push(result.east);
            }

            // South West
            if ((indexRow + i) < rows && (indexCol - i) >= 0) {
                result.southwest = this.imageData.data[((indexRow + i) * rowLength) + ((indexCol + i) * 4) + channel];
                result.arr.push(result.southwest);
            }

            // South
            if ((indexRow + i) < rows) {
                result.south = this.imageData.data[((indexRow + i) * rowLength) + (indexCol * 4) + channel];
                result.arr.push(result.south);
            }

            // South East
            if ((indexRow + i) < rows && (indexCol + i) < cols) {
                result.southeast = this.imageData.data[((indexRow + i) * rowLength) + ((indexCol + i) * 4) + channel];
                result.arr.push(result.southeast);
            }
        }
